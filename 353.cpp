...
public:
    ...
    SnakeGame(int width, int height, vector<pair<int, int>> food) {
        ...
        for (int i = 0; i < food.size(); i++) {
            q.push(food[i].first * w + food[i].second);
        }
        snake.push_back(0);
        s.insert(0);
    }
    ...
    int move(string direction) {
        int curr = snake.front();
        int curr_h = curr / w;
        int curr_w = curr % w;
        if (direction == "U") curr_h--;
        else if (direction == "D") curr_h++;
        else if (direction == "L") curr_w--;
        else curr_w++;
        
        if (curr_h < 0 || curr_h >= h || curr_w < 0 || curr_w >= w) return -1;
        else {
            int hv = curr_h * w + curr_w;
            if (!q.empty() && q.front() == hv) {
                q.pop();
            }
            else {
                s.erase(snake.back());
                snake.pop_back();
                if (s.count(hv) > 0) return -1;
            }
            snake.push_front(hv);
            s.insert(hv);
        }
