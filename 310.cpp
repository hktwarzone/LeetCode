...
public:
    vector<int> findMinHeightTrees(int n, vector<pair<int, int>>& edges) {
        ...
        for (int i = 0; i < n; i++) {
            if (matrix[i].size() == 1) current.push_back(i);
        }
        while(!current.empty()) {
            for (int i = 0; i < current.size(); i++) {
                for (auto it = matrix[current[i]].begin(); it != matrix[current[i]].end(); it++) {
                    matrix[*it].erase(current[i]);
                    if (matrix[*it].size() == 1) next.push_back(*it);
                }
            }
            if (next.empty()) break;
            current.clear();
